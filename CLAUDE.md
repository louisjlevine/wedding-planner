...
- **Tasks are fully editable in place** — `Timeline.tsx`'s row editor owns title, assignee, category, priority, date, and notes in one Save. `Task.assignee` is deliberately free text (`assigneeSuggestions()` only supplies the quick-pick chips), so anyone outside the couple can own a task. `Task.notes` is free-form text, previewed on the row (2-line clamp) but only editable inside the editor.
- **Removal lives in the editor and is tombstoned** — any task can be removed, adapter-derived ones included, via the two-step Remove control inside the row editor (never a button on the row itself). Dropping a seed task from `tasks` isn't enough — `buildInitialTasks` would re-seed it — so `removeTask` records the id in `removedTaskIds` and `mergePlanTasks` filters both sides. Any new read path over the plan must pass `removedTaskIds` through, and `importStore` unions the tombstones so a delete on one device survives a sync from another.
